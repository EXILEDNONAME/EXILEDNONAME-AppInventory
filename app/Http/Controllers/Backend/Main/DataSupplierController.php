...
use App\Http\Requests\Backend\Main\DataSupplier\DataSupplierStoreRequest;
use App\Http\Requests\Backend\Main\DataSupplier\DataSupplierUpdateRequest;
...
class DataSupplierController extends Controller {
  ...
  **/
  ...
  public function store(DataSupplierStoreRequest $request) {
    $store = $request->all();
    $this->model::create($store);
    return redirect($this->url)->with('success', trans('default.notification.success.item-created'));
  }

  public function update(DataSupplierUpdateRequest $request, $id) {
    $data = $this->model::findOrFail($id);
    $update = $request->all();
    $data->update($update);
